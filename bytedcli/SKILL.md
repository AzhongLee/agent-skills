...
9. 若本地 SSO / JWT override 不可用，通用 ByteCloud JWT 获取会按 `BYTEDCLI_USER_CLOUD_JWT -> AIME_USER_CLOUD_JWT` 回退；Codebase 会按 `BYTEDCLI_USER_CODE_JWT -> AIME_USER_CODE_JWT -> PAT -> 当前 SSO 会话自动兑换 Codebase JWT` 回退。需要单独取出 Codebase JWT 时，可执行 `auth get-codebase-jwt-token`。
...
# 在 code.byted.org 的 Git 仓库目录内也可省略 -R/--repo，Codebase 会从当前 origin 自动推断仓库
