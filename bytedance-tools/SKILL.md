...
description: "Router skill for bytedcli (ByteDance internal CLI). Use when tasks mention bytedcli, SSO/auth, Feishu/Lark docs or collaboration workflows, TCE/TOS/Netlink/Neptune, RDS, ByteDoc, TCC, SCM, Cloud Docs, ByteTech, Cloud Ticket, ENV, Cronjob, Overpass, APM, Slardar Web alarms, BAM, AGW, Log, Cache, Hive, Dorado, Aeolus, ES, Goofy Deploy, Goofy Preview, `goofy deploy`, `goofy preview`, quick preview, local web deployment, BMQ, Kafka, BITS, BitsAI engineering knowledge or engineering asset Q&A, DKMS, KMS v2, IAM, data encryption keys, dev tasks, pipelines, releases, or internal tech articles. Routes to subskills by domain and provides common execution patterns with --json output and help handling."
---
...
- 需要在 RDS/ByteDoc/TCC/TCE/TOS/Netlink/Neptune/SCM/Codebase/Cloud Docs/ByteTech/Cloud Ticket/ENV/Cronjob/Overpass/APM/BAM/Log/Cache/Hive/Dorado/Aeolus/ES/Goofy Deploy / Goofy Preview/BMQ/DKMS/KMS v2/IAM 之间路由
- 需要直接调用 Slardar Web `alarm-rule-list` / `alarm-history`，或分析告警页 URL、整理告警规则指标，并把结构化结果交给后续 agent 自己组合
...
如果调用方使用的是本地 npm 全局安装，而不是 `npx @latest`，升级到最新版本可直接执行：
...
- **Dorado（数据任务、执行 Hive SQL / HSQL 临时查询）**：`bytedance-dorado`
...
- **APM**：`bytedance-apm`
- **Slardar Web**：`bytedance-slardar-web`
- **BAM**：`bytedance-bam`
- **AGW (API Gateway)**：`bytedance-agw`
- **Log**：`bytedance-log`
...
- 需要排查具体请求链路时，优先使用全局参数 `--http-debug`；需要精细控制输出内容时，用 `--http-print <parts>`（支持 `H/B/h/b/m/t`），也可以通过 `--http-trace-file <path>` 把 trace 落盘并用 `--http-body-limit <bytes>` 控制 body 预览长度
