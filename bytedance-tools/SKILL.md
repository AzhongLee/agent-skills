...
description: "Router skill for bytedcli (ByteDance internal CLI). Use when tasks mention bytedcli, SSO/auth, TCE/TOS/Netlink/Neptune, RDS, TCC, SCM, Cloud Docs, ByteTech, Cloud Ticket, ENV, Overpass, APM, BAM, Log, Cache, Hive, DataLeap, BMQ, Kafka, BITS, dev tasks, pipelines, releases, or internal tech articles. Routes to subskills by domain and provides common execution patterns with --json output and help handling."
---
...
- 需要在 RDS/TCC/TCE/TOS/Netlink/Neptune/SCM/Codebase/Cloud Docs/ByteTech/Cloud Ticket/ENV/Overpass/APM/BAM/Log/Cache/Hive/BMQ 之间路由
...
- 缺少必填参数会输出完整帮助信息
- 需要机器可读输出时加 `--json`
- `--json` 是全局参数，必须放在子命令前，例如 `--json auth status`
